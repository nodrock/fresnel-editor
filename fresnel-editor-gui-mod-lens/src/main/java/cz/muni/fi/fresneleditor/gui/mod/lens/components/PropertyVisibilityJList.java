...
public class PropertyVisibilityJList extends
	...
	private static class PropertyVisibilityLMCellRenderer extends
		...
		private PropertyVisibilityLMCellRenderer() {
		}
			...
			if (value instanceof PropertyVisibilityWrapper) {
				...
				if (component instanceof JLabel) {
					...
					if (propVisibility.isComplexPropertyDescription()) {
						...
						String complexPropertyLabel = getComplexPropertyLabel(propVisibility);
						label.setText(complexPropertyLabel);
					} else {
						// set one line text
						label.setText(FresnelUtils.replaceNamespace(
								propVisibility.getFresnelPropertyValueURI(),
								ContextHolder.getInstance()
										.getBothDaosNamespaces()));
					}

					label.setToolTipText(propVisibility
		 ...
		 * @return
		 */
		private String getComplexPropertyLabel(
