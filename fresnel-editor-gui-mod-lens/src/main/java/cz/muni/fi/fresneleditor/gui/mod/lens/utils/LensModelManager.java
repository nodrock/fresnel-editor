...
import cz.muni.fi.fresneleditor.gui.mod.lens.LensModel;
...
public class LensModelManager extends AModelManager<Lens> {
	...
	@Override
	public SesameLens convertModel2JFresnel(IModel model) {
		...
		SesameLens sesameLens = (SesameLens) myRepository.getLens(model
				.getModelUri());
		...
		if (lensModel.getGroups() != null && !lensModel.getGroups().isEmpty()) {
			sesameLens.addAssociatedGroups(new Vector<Group>(lensModel
					.getGroups()));
		}
		return sesameLens;
